        ...
        CurrentAppService
            ...
            .then(function (currentApp) {
                ...
                $scope.toTime = $moment($scope.user.lastSession)
                    ...
                    console.log($moment($scope.user.lastSession).unix() * 1000, $scope.user.lastSession);
                $scope.fromTime = $moment($moment($scope.user.lastSession).unix() * 1000 - 24 *
                    60 * 60 *
                    1000)
                    .startOf('day')
                    .unix();
                ...
                var populateEvents = function (err, events) {
                    ...
                    if (_.keys(events)
                        ...
                        $scope.lastEventDate = $moment($scope.lastEventDate).unix()*1000 -
                            24 * 60 * 60 *
                            1000;
                ...
                $scope.showEventNextDate = function () {
                    console.log("fromTime toTime: ", $scope.fromTime, $scope.toTime);
                    $scope.fromTime = $moment($scope.fromTime * 1000 -
                        24 * 60 *
                        60 * 1000)
                        .startOf('day')
                        .unix();
                    $scope.toTime = $moment($scope.toTime * 1000 - 24 *
                        60 * 60 *
                        1000)
                        .startOf('day')
                        .unix();
                    console.log("new fromTime: ", $scope.fromTime,
                        $moment($scope.fromTime *
                            1000)
                        .format());
                    console.log("new toTime: ", $scope.toTime, $moment(
                            $scope.toTime *
                            1000)
                        .format());
                ...
                $scope.populateMsgOnCreation = function (err, lastMsg) {
                    ...
                    console.log("subject created msg: ", lastMsg[0].sub,
                        lastMsg[0]
                ...
                $scope.openNoteModal = function () {
                    ...
                    modalInstance.result.then(function (note) {
                        ...
                        UserModel.createNote($scope.currentApp._id,
                            $scope.user
                            ._id,
                            data,
                ...
                UserModel.getUserProfile($stateParams.id, currentAppId,
                    populatePage);
