...
module.exports = (function () {
  ...
  // Define all configuration that is common to all environments
  var defaults = {
    ...
    // Otherwise it falls back to port 8000.
    //
    ...
    config: {
      paths: {
        environments: path.join(__dirname, 'environments')
      }
    }
  };

  var envConfigPath = path.join(defaults.config.paths.environments,
    defaults.environment + '.js');
  var environment = {};

  if (fs.existsSync(envConfigPath)) {
    environment = require(envConfigPath);
    console.log('Loaded environment config for',
      defaults.environment);
  } else {
    console.log('ERROR: Environment config for', defaults.environment,
      'not found.');
  }
